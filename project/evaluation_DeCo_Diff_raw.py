...
def evaluation_annotated_images(args, diffusion, model, vae):
    ...
    #_, _ = process_split_irregular_with_checkpoint(
    #    loader,
    #    args.split,
    #    diffusion,
    #    model,
    ...
    try:
        for idx, (x, seg, object_cls, image_paths, anomaly_classes, patch_coords) in enumerate(
            ...
            with torch.no_grad():
                ...
                # -----------------------------------------------------------------
                # Core computations (always needed)
                # -----------------------------------------------------------------
                ...
                # -----------------------------------------------------------------
                # Composite anomaly maps (always needed)
                # -----------------------------------------------------------------
                ...
                # -----------------------------------------------------------------
                # Optional computations (only when detailed reporting is needed)
                # -----------------------------------------------------------------
                if enable_excel_report or enable_save_optional_image_results:
                    # Decode intermediate latent samples for detailed analysis
                    image_samples_list = []
                    ...
                    # Additional difference maps for detailed reporting
            ...
            for b in range(batch_size):
                ...
                # Convert pixel coordinates to grid coordinates for comparison with ground truth
                ...
                if enable_excel_report or enable_save_optional_image_results:
                    # Create base record with always-available fields
                    optional_rec_fields = {
                        "split": ("meta", split),
                        "image_path": ("meta", image_paths[b]),
                        "anomaly_class": ("meta", anomaly_classes[b]),
                    ...
                    # Add optional fields only when detailed reporting is enabled
                    if enable_excel_report or enable_save_optional_image_results:
                        optional_rec_fields.update({
                    ...
                    optional_rec = make_record(**optional_rec_fields)
                ...
                if checkpoint_manager:
                    ...
                    try:
                        save_patch_results_from_records(
                            ...
                            patch_size,
                            x_coord, y_coord  # patch coordinates
                        )
                    except Exception as e:
    ...
    try:
        for idx, (x, seg, object_cls, image_paths, anomaly_classes, patch_coords) in enumerate(
            ...
            for b in range(batch_size):
                ...
                file_info = path_to_safe_filename(image_paths[b])
